		...
		if strings.HasPrefix(line, "#EXTINF:") {
			...
			if len(matches) == 5 {
				...
				for scanner.Scan() {
					...
					channels[currentChannel.Name] = currentChannel
					break
				}
			}
		}
...
func downloadPlaylistFile(channel Channel, dirPath string) error {
	...
	tempFile.Close()
	channelDirPath := filepath.Join(dirPath, "channels", strings.Replace(channel.Name, "/", "_", -1))
...
func saveChannelInfo(channel Channel, dirPath string) error {
	...
	channelInfoPath := filepath.Join(dirPath, "channels", strings.Replace(channel.Name, "/", "_", -1), "channel.json")
...
func setMediaURLs(channel *Channel, playlistPath string) error {
	...
	channelDir := filepath.Join(playlistPath, "channels", strings.Replace(channel.Name, "/", "_", -1))
